...
Deno.test("[0,1,0,1,0]", () => {
  const result = trap([0, 1, 0, 1, 0]);
  assertEquals(result, 1);
});

Deno.test("[2,0,2]", () => {
  const result = trap([2, 0, 2]);
  assertEquals(result, 2);
});

Deno.test("[0,1,0,2,1,0,1,3,2,1,2,1]", () => {
  const result = trap([0, 1, 0, 2, 1, 0, 1, 3, 2, 1, 2, 1]);
  assertEquals(result, 6);
});

Deno.test("[4,2,0,3,2,5]", () => {
  const result = trap([4, 2, 0, 3, 2, 5]);
  assertEquals(result, 9);
});

Deno.test("Should be quick even for a large array", () => {
  const input = [
    10527,
    740,
    9013,
  ...
  const mark = performance.mark("BigTest");
  trap(input);
  performance.mark("EndBigTest");
  const measure = performance.measure("Test", "BigTest", "EndBigTest");
  console.log(`Took ${measure.duration} ms's`);
  assertLess(measure.duration, 50);
});
